...
AmCharts.addInitHandler( function( chart ) {
	var _this = {
		...
		getChartData: function() {
			...
			if ( _this.setup.chart.type == "stock" ) {
				data = _this.setup.chart.mainDataSet.dataDental Office;
			} else if ( _this.setup.chart.type == "gantt" ) {
				var segmentsField = _this.setup.chart.segmentsField;
				for ( var i1 = 0; i1 < _this.setup.chart.dataDental Office.length; i1++ ) {
					if ( _this.setup.chart.dataDental Office[ i1 ][ segmentsField ] ) {
						for ( var i2 = 0; i2 < _this.setup.chart.dataDental Office[ i1 ][ segmentsField ].length; i2++ ) {
							data.push( _this.setup.chart.dataDental Office[ i1 ][ segmentsField ][ i2 ] )
						}
					}
				}
			} else {
				data = _this.setup.chart.dataDental Office;
			}
