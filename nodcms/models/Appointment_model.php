...
class Appointment_model extends CI_Model
{
    ...
    function getTimePeriod($day_no,$service_id=null,$selectMin=null,$treatment_id=null){
        $this->db->select("*");
        $this->db->from('r_time_period');
        $this->db->join('treatments',"FIND_IN_SET(r_time_period.service_id,treatments.dentists_id) > '0'");
        $this->db->where('r_time_period.day_no',$day_no);
        if($treatment_id!=null){
            $this->db->where('treatments.id',$treatment_id);
        }else {
        $this->db->where('treatments.id',$this->session->userdata("treatment_id"));
        }
        if($service_id!=null){
            $this->db->where('service_id',$service_id);
            //$this->db->where('find_in_set('.$service_id.',treatments.dentists_id) !=0');
        } 
        if($selectMin!=null){
            $this->db->where('r_time_period.period_start_time <= "'.$selectMin.'"');
            $this->db->where('r_time_period.period_end_time > "'.$selectMin.'"');
        }
        $this->db->order_by('r_time_period.period_start_time',"ASC");
        $query = $this->db->get();
    ...
    function getAllServices()
    {
        ...
        $this->db->join('treatments','treatments.provider_id = r_services.provider_id','left');
//        $this->db->join('extensions','extensions.relation_id = service_id');
        $this->db->where('r_services.provider_id', $this->provider['provider_id']);
...
//        $this->db->where('extensions.status',1);
    ...
    function checkPrevReservation($start_time,$end_time,$prev_start_time,$prev_end_time)
    {
        if(empty($prev_start_time) || empty($prev_end_time)){
            $return = 1;
            return $return;
        } else {
            foreach($prev_start_time as $key=>$value){
                /*if(($start_time > $value && $start_time < $prev_end_time[$key]) || ($end_time < $value && $end_time > $prev_end_time[$key])){
                    $return = 0;
                    return $return;
                } else {
                    $return = 1;
                ...
                else if(($start_time > $value) && ($start_time >= $prev_end_time[$key])) 
                {
                    $return = 1;
                    return $return;
                }
...
//        $this->db->where('data_type','r_providers');
    ...
    function getCustomProvider($conditions = NULL, $limit = NULL, $offset = NULL){
		...
		if(!empty($conditions['search_dental_clinic'])){
		...
		}else if(!empty($conditions['provider_location'])){
        ...
        }else if(!empty($conditions['treatment_cat'])){
            $this->db->join('treatments','r_providers.provider_id = treatments.provider_id');
            $this->db->join('subcategory','treatments.subcategory = subcategory.subcat_id');
            //$this->db->join('subcategory');
    ...
    function getPaypalPaid($id, $conditions){
        ...
        if($conditions!=NULL) $this->db->where($conditions);
        $this->db->where('r_services.provider_id', $this->provider['provider_id']);
        $this->db->where('r_services.reservation_id', $id);
        $query = $this->db->get();
        return $query->result_array();
    }
