<?php if($data["payment_type"]!=1 || $data["price"] == 0){ ?>
...
<?php if(isset($data)){ ?>
    ...
    <div class="row static-info">
        ...
        <div class="col-xs-6 value"><mark><?php echo date("j F Y",$data["reservation_date_time"]); ?></mark></div>
    </div>
    <div class="row static-info">
...
<?php if($data["payment_type"]==1 && $data["price"] > 0){ ?>
    ...
    <p class="text-center">
        <a target="_parent" id="paypal-btn" class="btn btn-success" href="#">
            <?php echo _l("Payment with Paypal", $this); ?>
        </a>
    </p>
    <hr>
<?php }elseif($data["payment_type"]==2 && $data["price"] > 0){ ?>
