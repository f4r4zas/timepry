...
function mk_hpostform($action="",$name=null){
    ?>
    <form role="form" class="form-horizontal" <?=$name!=null?'name="'.$name.'" id="'.$name.'"':''?> method="post" action="<?=$action?>">
    <?php
}
...
function mk_vtext($name,$caption="",$value="",$options=''){
    ?>
    <div class="form-group ">
...
function mk_vtexteditor_shortkeys($id,$name,$caption="",$value="",$options='',$shortkeys=array()){
    ?>
    <div class="form-group ">
        <label for="<?=$id?>" class="control-label"><?=$caption?></label>
        <?php if(count($shortkeys)!=0){ ?>
        <p>
            <?php foreach($shortkeys as $item){ ?>
                <button type="button" class="btn btn-default btn-sm" onclick="insertAtTexteditor('<?=$id?>','<?php echo $item['value']; ?>');"><?php echo $item['label']; ?></button>
            <?php } ?>
        </p>
        <?php } ?>
        <textarea class="form-control ckeditor" id="<?=$id?>" name="<?=$name?>" <?=$options?>><?=$value?></textarea>
        <?php if(isset($help)){ ?><p><?php echo $help; ?></p><?php } ?>
    </div>
    <?php
}
    ...
    <div class="form-group " style="display: none;">
        ...
        <div class="col-lg-10 col-sm-10">
            <div class="md-radio-inline">
                <div class="md-radio">
                    <input value="<?=$value?>" type="radio" id="yes<?=$name?>" name="<?=$name?>" class="md-radiobtn" <?php echo $checked!=null?"checked":""?>>
                    <label for="yes<?=$name?>">
                        <span></span>
                ...
                <div class="md-radio">
                    <input type="radio" id="no<?=$name?>" name="<?=$name?>" class="md-radiobtn" <?php echo $checked==null?"checked":""?>>
                    <label for="no<?=$name?>">
                        <span></span>
                        <span class="check"></span>
                        <span class="box"></span>
...
function mk_hselect_faicon($name,$caption="",$options_data=null,$value=null){
    ...
    ?>
    <div class="form-group ">
        <label for="<?=$id?>" class="control-label col-lg-2"><?=$caption?></label>
        <div class="col-lg-10 col-sm-10">
            <div class="row">
                <div class="col-sm-8">
                    <div class="input-group">
                        <span class="input-group-addon"><i id="btn_icon_<?=$id?>" class="fa <?=$value?>"></i></span>
                        <input id="<?=$id?>" name="<?=$name?>" value="<?=$value?>" type="text" class="form-control" onkeyup="$('#btn_icon_<?=$id?>').attr('class','fa '+$(this).val())">
                    </div>
                </div>
                <div class="col-sm-4">
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu" style="width: 100%;">
                            <div class="select_icon scroll-box" style="overflow: hidden;max-height: 300px;">
                                <label <?=($value==null)?'class="checked"':''?>><input type="radio" name="radio_<?=$name?>" value="" onclick="$('input[type=radio]:not(:checked)').parent().removeClass('checked'); $(this).parent().addClass('checked'); $('#btn_icon_<?=str_replace(array("[","]"),"",$name)?>').attr('class',$(this).next().attr('class'))" <?=($value==null)?"checked":""?>> <i class="fa fa-stop fa-2x text-danger"></i></label>
                                <?php if($options_data!=null){ ?>
                                    <?php foreach($options_data as $data){ ?>
                                        <label <?=($value!=null && $value==$data)?'class="checked"':''?>><input type="radio" name="radio_<?=$name?>" value="<?=$data?>" onclick="$('input[type=radio]:not(:checked)').parent().removeClass('checked'); $(this).parent().addClass('checked'); $('#<?=$id?>').val($(this).val()); $('#btn_icon_<?=$id?>').attr('class','fa '+$(this).val())" <?=($value!=null && $value==$data)?"checked":""?>> <i class="fa <?=$data?> fa-2x"></i></label>
                                    <?php } ?>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
                    <a href="http://fortawesome.github.io/Font-Awesome/icons/" target="_blank" class="btn btn-link"><i class="fa fa-external-link"></i></a>
                </div>
            </div>
        </div>
    </div>
