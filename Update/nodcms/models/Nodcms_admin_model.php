...
class Nodcms_admin_model extends CI_Model
{
    ...
    function check_auto_messages($language_id,$key){
        $this->db->select("count(*)");
    ...
    function edit_auto_messages($language_id,$key,$data){
        $this->db->where('language_id',$language_id);
        $this->db->where('code_key',$key);
//        $data['content']= $data['content'];
//        $data['content']= $data['content'];
        $this->db->update('auto_email_messages', $data);
    }
    function insert_auto_messages($language_id,$key,$data){
        $data['language_id']= $language_id;
        $data['code_key']= $key;
//        $data['content']= $content;
        $this->db->insert('auto_email_messages', $data);
    }
    function get_all_auto_messages($key, $conditions = NULL){
        $this->db->select("*");
        $this->db->where('code_key',$key);
        $this->db->from('auto_email_messages');
        if($conditions!=NULL) $this->db->where($conditions);
